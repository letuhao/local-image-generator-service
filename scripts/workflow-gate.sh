...
# Prefer a python that can handle multi-line `-c` input. On some Windows
# setups (pyenv-win) the `python3` shim mangles multi-line arguments, so
# we probe each candidate with a tiny multi-line test first.
PYTHON_CMD=""
_py_works() {
  local cmd="$1"
...
for _candidate in python python3; do
  if _py_works "$_candidate"; then
    PYTHON_CMD="$_candidate"
    break
  fi
done
if [[ -z "$PYTHON_CMD" ]]; then
  echo "ERROR: No working python found (tried: python, python3). Install Python 3.x."
  exit 1
fi
